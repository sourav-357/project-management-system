...
/**
 * Handle document file upload directly to Cloudinary from memory buffer
 */
export const uploadProjectFile = async (fileBuffer, originalName, mimeType, projectId) => {
  try {
    const resourceType = mimeType.startsWith('image/') ? 'image' : 'raw';
    const uploadResult = await uploadToCloudinary(fileBuffer, `academic_platform/projects/${projectId}`, resourceType);
...
/**
 * Stream or download project deliverable file by redirecting directly to Cloudinary CDN URL
 */
