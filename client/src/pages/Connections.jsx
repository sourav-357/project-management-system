...
import {
  Users, UserCheck, UserPlus, UserMinus, UserX, Clock, ShieldOff, Check, X, Ban, Search, Filter, AlertCircle, CheckCircle2, RefreshCw, MessageSquare, Trash2, ArrowRight, Sparkles
} from 'lucide-react';
...
export const Connections = () => {
  ...
  useEffect(() => {
    // Always fetch pending count for tab badge
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-slate-950 via-indigo-950 to-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
        <div className="absolute right-0 top-0 translate-x-8 -translate-y-8 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 relative z-10">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/20 text-indigo-200 text-[11px] font-bold mb-2 border border-indigo-500/30">
              <Sparkles className="w-3.5 h-3.5 text-indigo-400" /> Academic Peer Network
            </div>
            <h1 className="text-2xl font-extrabold tracking-tight">Connections & Directory</h1>
            <p className="text-xs text-slate-300 mt-1">Manage your academic peers, pending requests, history, and blocked directory.</p>
          </div>
      ...
      {msg && (
        <div className="p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-xl text-emerald-800 dark:text-emerald-300 text-xs flex items-center justify-between gap-2 shadow-xs animate-fadeIn">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
            <span className="font-semibold">{msg}</span>
          </div>
          <button onClick={() => setMsg('')} className="text-slate-400 hover:text-slate-600"><X className="w-3.5 h-3.5" /></button>
        </div>
      )}

      {error && (
        <div className="p-4 bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 rounded-xl text-rose-800 dark:text-rose-300 text-xs flex items-center justify-between gap-2 shadow-xs animate-fadeIn">
          <div className="flex items-center gap-2">
            <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
            <span className="font-semibold">{error}</span>
          </div>
          <button onClick={() => setError('')} className="text-slate-400 hover:text-slate-600"><X className="w-3.5 h-3.5" /></button>
        </div>
      )}
      ...
      <div className="flex overflow-x-auto border-b border-slate-200 dark:border-slate-800 no-scrollbar">
        ...
        >
          ...
          {incoming.length > 0 && (
            <span className="w-5 h-5 rounded-full bg-rose-600 text-white text-[10px] flex items-center justify-center font-extrabold animate-pulse">
              {incoming.length}
            </span>
          )}
        </button>
      ...
      {activeTab === 'my-connections' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xs font-extrabold text-slate-900 dark:text-slate-100 uppercase tracking-wider flex items-center gap-2">
              <UserCheck className="w-4 h-4 text-emerald-600" /> Active Friends & Peers ({connectedUsers.length})
            </h2>
          </div>
          ...
          {connectedLoading ? (
            ...
            <div className="bg-white dark:bg-slate-900 p-12 rounded-2xl border border-slate-200 dark:border-slate-800 text-center text-slate-400 space-y-3 shadow-xs">
              ...
              <p className="text-[11px] max-w-sm mx-auto">Explore the peer network to send connection requests to students, supervisors, and admins.</p>
              <button
                onClick={() => setActiveTab('explore')}
                className="px-4 py-2 bg-indigo-600 text-white font-bold text-xs rounded-xl hover:bg-indigo-700 transition-all inline-flex items-center gap-1.5"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {connectedUsers.map((u) => (
                <div
                  key={u._id}
                  className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md transition-all relative group"
                >
                  <div className="flex items-start gap-3">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold flex items-center justify-center text-sm border border-slate-200 dark:border-slate-700 overflow-hidden shrink-0 shadow-sm">
                      {u.avatar ? (
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-xs font-extrabold text-slate-900 dark:text-slate-100 truncate">{u.name}</h3>
                      <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{u.email}</p>
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className="px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900 text-[10px] font-bold">
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2">
                    <button
                      onClick={() => navigate('/chat')}
                      className="flex-1 py-1.5 px-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm"
                    >
                    ...
                    <button
                      ...
                      className="p-2 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-xl transition-all border border-slate-200/80 dark:border-slate-800"
            ...
            <div className="flex gap-1.5 shrink-0">
              {['All', 'Student', 'Teacher', 'Admin'].map((r) => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-extrabold rounded-xl transition-all border ${
                    roleFilter === r
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    {u.connectionStatus === 'accepted' ? (
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 bg-indigo-600 text-white font-bold text-xs rounded-xl hover:bg-indigo-700 transition-all flex items-center gap-1.5 shadow-sm"
                      >
      ...
      {activeTab === 'pending' && (
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="text-xs font-extrabold text-slate-900 dark:text-slate-100 uppercase tracking-wider flex items-center gap-2">
                <Clock className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Incoming Connection Requests ({incoming.length})
              </h2>
              {incoming.length > 0 && (
                <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/80 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-900">
                  Action Required
                </span>
              )}
            </div>
            ...
            {incoming.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 text-center text-slate-400 text-xs shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {incoming.map((req) => (
                  <div
                    key={req._id}
                    className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-l-4 border-l-indigo-600 dark:border-l-indigo-500"
                  >
                    <div className="flex items-center gap-3.5">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-purple-600 text-white font-extrabold flex items-center justify-center text-sm border border-white/20 overflow-hidden shrink-0 shadow-md">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950/80 text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900">
                            {req.requester?.role}
                          </span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">
                            &bull; {req.requester?.department || 'Computer Science'}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800/80 shrink-0">
                      <button
                        onClick={() => handleRespond(req._id, 'accept')}
                        className="flex-1 sm:flex-initial px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-extrabold text-xs rounded-xl shadow-md shadow-emerald-900/10 transition-all flex items-center justify-center gap-1.5"
                      >
                        <Check className="w-3.5 h-3.5" /> Accept
                      </button>
                      <button
                        onClick={() => handleRespond(req._id, 'reject')}
                        className="flex-1 sm:flex-initial px-3.5 py-2 bg-rose-50/80 dark:bg-rose-950/50 hover:bg-rose-100 dark:hover:bg-rose-900/80 text-rose-700 dark:text-rose-300 border border-rose-200/80 dark:border-rose-900/80 font-extrabold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5"
                      >
          ...
          <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
            ...
            {outgoing.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 text-center text-slate-400 text-xs shadow-xs">
                ...
                {outgoing.map((req) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-extrabold text-slate-900 dark:text-slate-100">{req.recipient?.name}</h4>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-1 bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-300 text-[10px] font-bold rounded-xl">
                        Pending
                      </span>

                      <button
                        onClick={() => handleCancelOutgoingRequest(req.recipient._id || req.recipient, req.recipient?.name)}
                        className="px-3 py-1.5 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 dark:hover:bg-rose-900 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900 font-bold text-xs rounded-xl transition-all flex items-center gap-1"
            ...
            {history.length === 0 ? (
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
                {history.map((item) => {
                  ...
                  return (
                    <div key={item._id} className="p-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-extrabold text-slate-900 dark:text-slate-100">{partner?.name}</p>
                ...
                {blockedUsers.map((bUser) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
                    >
                      Unblock
                    </button>
                  </div>
